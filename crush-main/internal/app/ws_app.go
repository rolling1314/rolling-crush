...
func (app *WSApp) getSessionContextWindow(ctx context.Context, sessionID string) int64 {
	...
	if models, ok := configData["models"].(map[string]interface{}); ok {
		slog.Info("Found models in config", "session_id", sessionID, "models_keys", wsGetKeys(models))
...
// wsGetKeys is a helper function to get map keys for logging
func wsGetKeys(m map[string]interface{}) []string {
